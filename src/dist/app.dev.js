...
app.use(bodyParser.json({
  limit: '20mb'
})); // atur limit lebih besar sesuai kebutuhan
...
app.use(morgan('dev'));
app.use(cors()); // Database connect & sync
...
sequelize.authenticate().then(function () {
  ...
  return console.log('✅ DB connected & synced');
})["catch"](function (err) {
  return console.error('❌ DB error:', err);
}); // Swagger UI

var swaggerDocument = require('./config/swagger-docs');
...
app.use('/api/v1/upload', require('./features/upload/uploadRoutes')); // Error handler
